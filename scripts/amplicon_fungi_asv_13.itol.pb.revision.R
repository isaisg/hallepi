...
df_res$Class <- paste0("c_",df_res$Class)
df_res$Order <- paste0("o_",df_res$Order)
df_res$Family <- paste0("f_",df_res$Family)
df_res$Genus <- paste0("g_",df_res$Genus )
...
dir.create(outdir)
write.tree(phy = tree,file = "../cleandata/itol/386_asvs_fungi.newick")



#Colored ranges
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_otuslevel_colored_ranges.txt"
...
palette_fungi <- readRDS(file = "../rawdata/hallepi_palette_fungi_orders.RDS")
...
#Colored ranges
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_otulevel_labelbranch_colors.txt"
mline <- c("DATASET_STYLE","Separator TAB","DATASET_LABEL\tOrder Labels","COLOR\tred",
           ...
           "LEGEND_COLORS\t#c9d73d\t#c555cb\t#cb7c77\t#c49a3f\t#5d8d9c\t#68d359\t#6b42c8\t#aed688\t#502e71\t#6a7dc9\t#722e41\t#d7652d\t#7cd5c8\t#c5383c\t#507d41\t#cf4c8b\t#c8b693\t#674c2a\t#c6a5cc\tblack",
           "LEGEND_LABELS\tAscomycota_unclassified\tBranch06\tCapnodiales\tChaetothyriales\tGlomerellales\tHelotiales\tHypocreales\tPezizales\tPleosporales\tSordariales\tThelebolales\tXylariales\tAgaricales\tCantharellales\tCystofilobasidiales\tFilobasidiales\tPlatygloeales\tMortierellales\tFungi_unclassified\tOther",
           "DATA")
write(x = mline,file = outfile,append = F,sep = "")
...
df_temp$log2FoldChange[which(df_temp$log2FoldChange<=-10)] <- -10
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_otulevel_heatmap_rootsoil.txt"
...
df_temp$log2FoldChange[which(df_temp$log2FoldChange>= 10)] <- 10
df_temp$log2FoldChange[which(df_temp$log2FoldChange<=-10)] <- -10
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_otulevel_heatmap_shootsoil.txt"
...
df_temp$log2FoldChange[which(df_temp$log2FoldChange>= 10)] <- 10
df_temp$log2FoldChange[which(df_temp$log2FoldChange<=-10)] <- -10
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_otulevel_heatmap_shootroot.txt"
...
getpopupinfo <- function(df = df_tax, contrasts = NULL,thres_padj = 0.1){
  ...
  Res_num <- NULL
  for(id in ids){
    nt <- df %>% subset(pathString == id) %$% Id %>% unique %>% length
    ...
    for(contrast in contrasts){
      ...
      ntotal <- df_temp$Id %>% unique %>% length
...
outfile <- "../cleandata/itol/itol_amplicon_fungi_asvs_asvlevel_popup.txt"
