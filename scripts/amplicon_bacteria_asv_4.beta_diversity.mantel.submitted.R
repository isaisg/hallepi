...
capture.output(file = "../figures/supfigure2_mantel_root_shoot.doc",
               append = F,print(m1))

df1 <- melt_dist(dist = dist_root %>% as.matrix) 
df2 <- melt_dist(dist = dist_shoot %>% as.matrix)

dfpval <- data.frame(r = m1$statistic %>% round(3) ,p = m1$signif %>% format.pval(digits = 2))
...
p <- ggplot(data = melted,aes(Root,Shoot)) + geom_point(shape = 21,,size = 5) + 
  ...
  geom_text(dfpval,mapping = aes(0.9,0.5,label =label),size = 5)

oh.save.pdf(p = p,outname = "mantel_bacteria_root_shoot_big.pdf",outdir = "../figures/",width = 10,height = 10)
...
m_root_shoot <- mantel(xdis = dist_root,ydis = dist_shoot,
                       permutations = 10000)
capture.output(file = "../figures/supfigure2_mantel_root_shoot_small.doc",
...
oh.save.pdf(p = p,outname = "mantel_bacteria_root_shoot_small.pdf",outdir = "../figures/",width = 10,height = 10)



m_root_soil <- mantel(xdis = dist_root,ydis = dist_soil,
                      permutations = 10000)
capture.output(file = "../figures/supfigure2_mantel_root_soil_small.doc",
...
oh.save.pdf(p = p,outname = "mantel_bacteria_soil_root_small.pdf",outdir = "../figures/",width = 10,height = 10)
