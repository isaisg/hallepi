...
samples_remove <- Map$OTU_Header %>% as.character
...
#Root versus Shoot ###
samples_remove <- Dat_root$Map$OTU_Header[which(!(Dat_root$Map$Pot %in% int_root_shoot))] %>%
  as.character
Dat_root <- remove_samples(Dat = Dat_root,samples = samples_remove,droplevels = T)
samples_remove <- Dat_shoot$Map$OTU_Header[which(!(Dat_shoot$Map$Pot %in% int_root_shoot))] %>%
  as.character
...
Tab_root <- match(Map_root$OTU_Header,colnames(Tab_root)) %>%
  Tab_root[,.]
Tab_shoot <- match(Map_shoot$OTU_Header,colnames(Tab_shoot)) %>%
  Tab_shoot[,.]
...
Tab_shoot <- match(Map_shoot$OTU_Header,colnames(Tab_shoot)) %>%
  Tab_shoot[,.]
Tab_soil <- match(Map_soil$OTU_Header,colnames(Tab_soil)) %>%
  Tab_soil[,.]
