...
df_tax <- readRDS(file = "../cleandata/dat_hallepi_amplicon_fungi_asvs_soil.2019.RDS") %$%
  df_tax
df_tax <- df_tax[,-2]
colnames(df_tax)[1] <- "ASV_Id"
suptab <- merge(all_res,df_tax, by = "ASV_Id")


suptab$Num <- suptab$ASV_Id %>% gsub(pattern = "ASV",replacement = "") %>% as.numeric
suptab$Genotype <- suptab$Genotype %>% factor(levels = c("Col-0","phf1","phr1/phl1","InterGenotype"))
suptab <- with(suptab,order(Num,Genotype)) %>% suptab[.,]
suptab <- suptab[,-19]

#Write table
write.table(x = suptab,file = "../cleandata/sup_table_7.csv",append = F,quote = F,sep = ",",
